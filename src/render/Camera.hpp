...
public:
    Camera() = default;
    ~Camera(){spdlog::debug("camera destruct");} ;

    glm::mat4 GetView() const{ return view_; };
    glm::mat4 GetProj() const{ return proj_; };

    void LookAt() { view_ = glm::lookAt(position_, position_ + front_, up_); };
    void Proj()
    {
    ...
    void MoveForward(float movement) { position_ += front_ * movement; };
    void MoveRight(float movement) { position_ += right_ * movement; };
    void MoveUp(float movement) { position_ += up_ * movement; };
