...
void CascadedShadowMap::CreateFBO()
{
    // configure light FBO
    // -----------------------
    ...
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_DEPTH_COMPONENT32F, depthMapResolution_, depthMapResolution_,
    ...
    // configure UBO
    // --------------------
    glCreateBuffers(1, &matricesUBO_);
    glNamedBufferStorage(matricesUBO_, sizeof(glm::mat4x4) * 16, nullptr, GL_DYNAMIC_STORAGE_BIT);
    glBindBufferBase(GL_UNIFORM_BUFFER, 2, matricesUBO_);
}

void CascadedShadowMap::Init(const Camera &_camera, const glm::vec3 &_lightDir, const std::shared_ptr<Shader> &_shader,
                             const std::shared_ptr<Shader> &_shader_skeletal)
{
    shader_ = _shader;
    shader_skeletal_ = _shader_skeletal;
    ...
    shadowCascadeLevels_ = std::vector<float>{cameraFarPlane_ / 50.0f, cameraFarPlane_ / 25.0f, cameraFarPlane_ / 10.0f,
                                              cameraFarPlane_ / 2.0f};
...
void CascadedShadowMap::Use(const Camera &_camera)
{
    cameraProj_ = _camera.GetProj();
    cameraView_ = _camera.GetView();
    ...
    {
        glNamedBufferSubData(matricesUBO_, i * sizeof(glm::mat4x4), sizeof(glm::mat4x4), &lightMatrices[i]);
    }
...
std::vector<glm::vec4> CascadedShadowMap::GetFrustumCornersWorldSpace(const glm::mat4 &projview)
{
...
{
    const auto proj = glm::perspective(glm::radians(zoom_), (float)fb_width_ / (float)fb_height_, nearPlane, farPlane);
...
std::vector<glm::mat4> CascadedShadowMap::GetLightSpaceMatrices()
{
